...
from util.pyutils import setup_logger
import torch
...
from model import PI_CLIP_deconder_GCN
...
def get_parser():
    ...
    parser.add_argument('--arch', type=str, default='PI_CLIP_deconder_GCN')
    parser.add_argument('--viz', action='store_true', default=False)
    parser.add_argument('--config', type=str, default='config/pascal/pascal_split0_resnet50_manet.yaml',
                        help='config file')
    # parser.add_argument('--config', type=str, default='config/pascal/coco_split0_resnet50.yaml',
    #                     help='config file')
    ...
    parser.add_argument("--work_dir", default="work_dir_clip_wseg", type=str, help="work_dir_clip_wseg")
...
def validate(val_loader, model):
    ...
    for e in range(db_epoch):
        for i, (input, input_name, target, target_b, s_input, s_mask, subcls, class_name, ori_label, ori_label_b,
            ...
            save_folder = 'save_outpng_shot5_split0_{}'.format(e)
            os.makedirs(save_folder, exist_ok=True)
            #
            for i, (tensor, image_path) in enumerate(zip(output, input_name)):
                #
                result = torch.argmax(tensor, dim=0)
                result_cpu = result.byte().cpu()
                result_np = result_cpu.numpy()
                result_np[result_np != 0] = 255
                ...
                new_file_name = f"{tmp_file_name}.png"
                image_save_path = os.path.join(save_folder, new_file_name)

                cv2.imwrite(image_save_path, result_np)
...
if __name__ == '__main__':
    ...
    setup_logger(filename=os.path.join(args.work_dir, 'pascal_split2_shot1_test.log'))
