...
from util.clip_text import BACKGROUND_CATEGORY
...
def generate_prompts(class_names, dataset_name):
    """
    为数据集的类别生成多样化的前景和背景描述。

    参数：
        class_names (list): 数据集的类别名称列表。
        dataset_name (str): 数据集名称（如 'pascal' 或 'coco'）。
    ...
    fg_templates = [
        ...
        "an image containing {}.",
        "a detailed view of {}.",
        "a scene with {} visible."
    ]
    bg_templates = [
        "a photo without {}.",
        "an image where {} is absent.",
        "a scene free of {}.",
        "a background without any {}."
    ]
    ...
    if dataset_name == 'pascal':
        fg_templates.extend([
            ...
            "a common view of {} in a real-world setting."
        ])
        bg_templates.extend([
            "a peaceful environment with no {}.",
            "a clear scene without any signs of {}."
        ])
    elif dataset_name == 'coco':
        fg_templates.extend([
            "an instance of {} in a typical setting.",
            "{} captured in a crowded scene."
        ])
        bg_templates.extend([
            "a bustling environment but no {} present.",
            "an urban view with no trace of {}."
        ])
    ...
    for class_name in class_names:
        fg_prompts.extend([template.format(class_name) for template in fg_templates])
        bg_prompts.extend([template.format(class_name) for template in bg_templates])

    return {"foreground": fg_prompts, "background": bg_prompts}
...
class OneModel(nn.Module):
    def __init__(self, args, cls_type=None):
        ...
        # 根据数据集生成描述
        # if self.dataset == 'pascal':
        #     prompts = generate_prompts(new_class_names, 'pascal')
        # elif self.dataset == 'coco':
        #     prompts = generate_prompts(new_class_names_coco, 'coco')
        #
        # # 使用生成的描述计算文本特征
        # self.bg_text_features = zeroshot_classifier(new_class_names, prompts["background"], self.clip_model)
        # self.fg_text_features = zeroshot_classifier(new_class_names, prompts["foreground"], self.clip_model)
