...
# def generate_cam(feature_map):
#     """
#     生成类激活图 (CAM) 从特征图 (1, 2, 60, 60)
#     1. 对每个通道计算 CAM。
#     2. 对 CAM 应用颜色映射并保存为热力图。
#     """
#
#     feature_map = feature_map.squeeze(0)  # (2, 60, 60)
#
#     cam_list = []
#
#     # 对每个通道生成 CAM
#     for c in range(feature_map.shape[0]):
#         cam = feature_map[0, :, :]  # 取第 c 个通道的特征图
#
#         # 使用 torch 的 min 和 max 进行归一化
#         cam_min = cam.min()
#         cam_max = cam.max()
#         cam = (cam - cam_min) / (cam_max - cam_min)  # 归一化
#
#         # 将 CAM 转换为热力图
#         heatmap = cm.coolwarm(cam.cpu().numpy())  # 使用 jet 色图
#         heatmap = np.delete(heatmap, 3, axis=2)  # 如果有 alpha 通道，删除它
#         heatmap = np.uint8(heatmap * 255)  # 转换为 0-255 范围
#
#         # 存储热力图
...
def generate_cam(feature_map):
    ...
    # 获取 coolwarm 色图的 RGB 值，使用 ScalarMappable
    norm = mcolors.Normalize(vmin=0, vmax=1)  # 归一化
    sm = cm.ScalarMappable(cmap=coolwarm, norm=norm)
    coolwarm_colors = sm.to_rgba(np.linspace(0, 1, 256))  # 获取 256 个 RGB 值

    # 反转 RGB 色图
    coolwarm_reversed = mcolors.ListedColormap(coolwarm_colors[::-1])
    cams = torch.zeros(feature_map.shape[0], feature_map.shape[1], feature_map.shape[2])
    # 对每个通道生成 CAM
    for c in range(feature_map.shape[0]):
        cam = feature_map[c, :, :]  # 取第 c 个通道的特征图
        # 使用 torch 的 min 和 max 进行归一化
        cam_min = cam.min()
        cam_max = cam.max()
        cam = (cam - cam_min) / (cam_max - cam_min)  # 归一化到 [0, 1] 范围
        cams += cam
    cams_min=cams.min()
    cams_max=cams.max()
    cam2 = (cams - cams_min) / (cams_max - cams_min)  # 归一化到 [0, 1] 范围
    heatmap = coolwarm_reversed(cam2.cpu().numpy())  # 使用反转的 coolwarm 色图
...
def multi_scale_optimization(img_init):
    ...
    # 融合不同尺度的图像 (简单加权平均)
    img_fused = (img_init + img_init_15 + img_init_30 + img_init_120) / 4

    # 通过上采样调整回原尺度 (1, 2, 60, 60)
    img_optimized = F.interpolate(img_fused, size=(60, 60), mode='bilinear', align_corners=False)
...
class OneModel(nn.Module):
    ...
    def forward(self, x, x_cv2, que_name, class_name, y_m=None, y_b=None, s_x=None, s_y=None, cat_idx=None):
        ...
        instance_prototype = supp_pro.view(bq, cq).unsqueeze(1).expand(bq, 256, cq)  # instance prototype
        ...
        #
        # cams = generate_cam(img_cam)
        #mg_c
        # for i, cam_img in enumerate(cams):
        #
        #     output_path = os.path.join(save_dir, f'cam_{que_name}.jpg')  # 使用索引 i 避免文件重名
        #     cv2.imwrite(output_path, cam_img)  # 保存为 JPG
