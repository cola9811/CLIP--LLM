...
import clip
import math
from model.get_cams import get_img_cam
from pytorch_grad_cam import GradCAM
from clip.clip_text import new_class_names, new_class_names_coco

def zeroshot_classifier(templates, model):
...
class OneModel(nn.Module):
    ...
    def forward(self, x, x_cv2, que_name, class_name, y_m=None, y_b=None, s_x=None, s_y=None, cat_idx=None):
        ...
        self.que_path = './data/VOCdevkit2012/VOC2012/JPEGImages'
        #LLM
        que_imgpath = os.path.join(self.que_path, que_name[0])

        with open(que_imgpath, 'rb') as img_file:
            img_base = base64.b64encode(img_file.read()).decode('utf-8')

        client = ZhipuAI(api_key="60dee0440b794b718768bcc1b17bb8be.H5hgXPR1IBtXkWX0")  # 填写您自己的APIKey
            ...
            messages=[
                {
                    ...
                    "content": [
                        ...
                        {
                            "type": "text",
                            "text": "Please describe the foreground and background of the image in two sentences, adhering to the requirement of one sentence for the foreground and one for the background.It would be:Describe the foreground of the image in one sentence. Describe the background in another sentence."
                        }
                    ]
                }
            ]
        ...
        bg_text_features = sentences[0].strip()
        fg_text_features = sentences[1].strip()
        self.bg_text_features = zeroshot_classifier(bg_text_features, self.clip_model)
        self.fg_text_features = zeroshot_classifier(fg_text_features, self.clip_model)
